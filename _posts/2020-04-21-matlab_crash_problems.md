...
理解事件驱动的编程方式 (Event-driven programming)
理解并能自觉运用 OOP (object oriented programming)
知道多线程、多进程
了解 HTTP 协议（数字货币 only）
其他通用的编程技能，例如：pdb 调试、单元测试、logging、版本控制 git、Linux 服务器常见操作、数据库交互等零零散散的知识点，以及读 api 文档，查 google，混社区和开发 QQ 群等软技能。
...
先行情接入，print 之。这个是最简单的，需要学习的就是 websocket。
跑通下单接口，然后做初步的封装。参考了交易所官方的代码实现。这一步主要解决的问题是登录 authentication 和 rest 请求加密。
订阅仓位和成交回报，在网页端手动下单测试。这个后面要用于 OMS(order management system)和独立风控模块。
...
当数据引擎(DataHandler)收到行情时(我们将其定义为市场事件 MarketEvent)，策略实例(strategy)将会据此执行策略计算函数 onMarketData()。策略计算函数可能会产生策略信号，我们定义其为策略事件 SignalEvent
组合对象 portfolio 收到策略信号，更新组合目标仓位，并据此下单，这些逻辑写在 SignalEvent 的回调函数 onSignal()中。下单请求发送成功后会生成一个订单事件 OrderEvent
portfolio 自身也订阅了 OrderEvent，其的回调函数 onOrderEvent()的功能主要是更新自身维护的组合持仓信息，将刚才的订单添加到未完成的订单列表中
protfolio 还订阅了组合仓位变动事件/成交事件 FillEvent（多数交易所都有这个功能：通过向交易所订阅成交或持仓信息，一个交易账号的成交信息或任何持仓变化都会不断推送过来，跟市场行情一样，我们把这些信息组装成标准格式的 FillEvent 推送动事件队列中），同样地，根据 FillEvent 跟新自身维护的组合持仓信息，另外如果订单未能在规定时间内成交，还需要追单从而生成新的 OrderEvent, FillEvent 直到实际仓位与目标仓位一致为止。
交易执行器 executor 订阅 OrderEvent 和 FillEvent，负责具体的下单逻辑。通常我们会将算法交易逻辑写在 executor 对象中。有些情况下 executor 也要订阅 MarketEvent，一种常见的下单方式是对一价下单，这就需要市场行情信息中的 orderbook 信息。
...
Event-Driven Backtesting with Python - QuantStart
Python 量化交易平台开发教程系列 4-事件驱动引擎原理和使用 - vnpy
编辑于 2018-12-06
...
data 为 post 字典的 url 编码。这里我们以市价单买入 20 手 XBTUSD 合约，url-encoding 为 ?symbol=XBTUSD&side=Buy&orderQty=20&ordType=Market 2.签名生成函数直接调用 python hmax 库:
```
import hmac
signature = hmac.new(apiSecret, msg, digestmod=hashlib.sha256).hexdigest() 3.请求过期时间要在程序中动态的生成，例如
...
strategy.on_depth(OrderbookDepthData) # 策略计算 2.通过 ws.run_forever()，启动一个 websocket 连接并让它持续运行。注意这个 ws 实例需要单开一个线程运行，否则程序会卡在 run_forever()这里。示例如下：（对多线程不熟悉的同学，自行 google 理解下面代码的含义）
```
import threading
